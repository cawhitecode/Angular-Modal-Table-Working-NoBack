...
import { MarketService, DoNotServiceCustomer } from '../../services/market.service';
...
export class DispatchComponent implements OnInit {
  ...
  public selectedMarkets: Array<Market> = [{marketId: 1001, gmtOffset: "-5", name: 'DFW - Dallas / Ft Worth', map: "Map URL Goes Here", DoNotServiceList: example }]
  ...
  constructor(private sa:MarketService) {
    setInterval(() => {
      this.date = new Date()
    }, 1000)
   }
  ...
  pushItemToSelectedMarkets(item){
    ...
    newmarket.DoNotServiceList = example;
  ...
  setTimzoneBasedOnItemId(item_id: number, newmarket: Market){
    ...
    console.log(newmarket.gmtOffset)
  }
}